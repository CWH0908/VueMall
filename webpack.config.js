...
module.exports={
	...
	module:{//此节点用于配置所有的第三方模块加载器
		rules:[//所有第三方模块的规则
			{ test:/\.css$/, use:['style-loader','css-loader']},//正则表达式匹配css后缀的文件
			{ test:/\.(jpg|jpeg|png|gif|bmg)$/, use:[{
																						loader:'url-loader',
																						options:{limit: 200,}
																					}]
			},  //正则表达式匹配图片后缀的文件
//			{test:/\.js$/,use:'babel-loader',exclude:/node_modules/},//配置babel来转换高级的ES6语法
			{ test:/\.vue$/, use:'vue-loader'},//正则表达式匹配vue后缀的文件
			{ test:/\.ttf$/, use:'url-loader'},//正则表达式匹配css后缀的文件
		]	
	}
}
